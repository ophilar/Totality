...
describe('Service Deep Dive 3 (No Mocks)', () => {
  ...
  beforeEach(() => {
    ...
    dbService.sourceRepo.upsertMediaSource({ source_id: 's1', source_type: 'local', display_name: 'S1', is_enabled: true } as any)
  })
  ...
  describe('WishlistCompletionService', () => {
    it('should complete missing items when added', async () => {
      ...
      dbService.mediaRepo.upsertMediaItem({
      ...
      // 4. Verify completed
      const items = dbService.getActiveWishlistItems()
  ...
  describe('DeduplicationService', () => {
    it('should detect duplicate movies', async () => {
      ...
      const dups = dbService.duplicateRepo.getPendingDuplicates()
      expect(dups.length).toBe(1)
      expect(dups[0].external_id).toBe('tmdb2')
    })

    it('should recommend the highest resolution', () => {
      const id1 = dbService.mediaRepo.upsertMediaItem({
      ...
      const id2 = dbService.mediaRepo.upsertMediaItem({
