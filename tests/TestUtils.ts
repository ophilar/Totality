...
const api: Record<string, unknown> & { __taskListeners: Array<(state: unknown) => void>; __triggerTaskQueueUpdate: (state: unknown) => void } = {
    ...
    getExclusions: (type: string, pKey?: string) => invoke(IPC_CHANNELS.DATABASE.GET_EXCLUSIONS, type, pKey),
    addExclusion: (type: string, key: string, parentKey?: string, title?: string, itemTitle?: string) => invoke(IPC_CHANNELS.DATABASE.ADD_EXCLUSION, type, key, parentKey, title, itemTitle),
    removeExclusion: (type: string, key: string, parentKey?: string) => invoke(IPC_CHANNELS.DATABASE.REMOVE_EXCLUSION, type, key, parentKey),
