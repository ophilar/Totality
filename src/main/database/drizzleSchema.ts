...
export const qualityScores = sqliteTable('quality_scores', {
  ...
  bitrateScore: integer('bitrate_score'),
  audioScore: integer('audio_score'),
  efficiencyScore: integer('efficiency_score'),
  storageDebtBytes: integer('storage_debt_bytes'),
  evidenceStatus: text('evidence_status').notNull().default('insufficient'),
  confidence: text('confidence').notNull().default('none'),
  savingsBasis: text('savings_basis').notNull().default('insufficient_data'),
...
export const musicQualityScores = sqliteTable('music_quality_scores', {
  ...
  tierQuality: text('tier_quality').notNull(),
  tierScore: integer('tier_score'),
  codecScore: integer('codec_score'),
  bitrateScore: integer('bitrate_score'),
  efficiencyScore: integer('efficiency_score'),
  storageDebtBytes: integer('storage_debt_bytes'),
  evidenceStatus: text('evidence_status').notNull().default('insufficient'),
...
export const libraryScans = sqliteTable('library_scans', {
  ...
  allowExpandedMatching: integer('allow_expanded_matching').notNull().default(0),
  createdAt: text('created_at').notNull(),
  updatedAt: text('updated_at').notNull(),
}, (table) => ({
