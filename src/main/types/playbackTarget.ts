...
export interface PlaybackTargetDefinition {
  containers: string[]
  video: { codecs: string[]; profiles: string[]; levels: number[]; maxWidth: number; maxHeight: number; maxFrameRate: number; bitDepths: number[] }
...
export function evaluatePlaybackTarget(profile: PlaybackTargetProfile, analysis: FileAnalysisResult): PlaybackTargetEvaluation {
  ...
  const findings = {
    container: finding(d.containers.includes(analysis.container), `container in [${d.containers.join(', ')}]`, analysis.container),
