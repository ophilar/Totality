...
export function PlaybackTargetProfilesTab() {
  ...
  const remove = async () => {
    ...
    if (selectedId === defaultProfileId) { setError('The default optimization profile cannot be deleted. Choose another default first.'); return }
  ...
  return <div className="grid min-h-0 grid-cols-1 gap-5 lg:grid-cols-[240px_1fr]">
    <section className="space-y-3"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Playback profiles</h2><p className="text-xs text-muted-foreground">Profiles used by optimization and compatibility evaluation.</p></div><button className="text-sm text-primary" onClick={createProfile}>New</button></div><div className="space-y-1">{profiles.map(profile => <div key={profile.id} className={`flex items-center gap-2 rounded-lg border p-2 ${selectedId === profile.id ? 'border-primary bg-primary/10' : 'border-border/50'}`}><button className="min-w-0 flex-1 p-1 text-left text-sm hover:text-foreground" onClick={() => selectProfile(profile)}><span className="block truncate">{profile.name}</span>{defaultProfileId === profile.id && <span className="text-[11px] text-primary">Default for optimization</span>}</button>{defaultProfileId !== profile.id && <button className="shrink-0 rounded border border-border px-2 py-1 text-[11px] hover:bg-muted" onClick={() => { void window.electronAPI.setSetting('optimization_default_target_profile_id', profile.id).then(() => setDefaultProfileId(profile.id)) }}>Set default</button>}</div>)}</div>{!profiles.length && <p className="rounded-lg border border-border/50 p-3 text-sm text-muted-foreground">No profiles yet.</p>}</section>
