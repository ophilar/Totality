...
export function ConversionRecommendation({
  item,
  compact = false,
}: {
  item: MediaItem
  ...
  const isInefficient = useMemo(() => {
    ...
    return (
      (score !== null && score !== undefined && score > 0 && score < 60) ||
  ...
  const heuristics = useMemo(() => {
    ...
    const isFilm =
      genres?.toLowerCase().includes('drama') || genres?.toLowerCase().includes('history')
  ...
  const handleAiAnalyze = async () => {
    ...
    try {
      const result = await window.electronAPI.aiCompressionAdvice({
        mediaId: item.id,
        requestId: `compression-${item.id}`,
      })
      ...
      window.electronAPI.log.error('[ConversionRecommendation]', 'AI Analysis failed:', error)
  ...
  return (
    <div
      className={`${baseClasses} bg-primary/5 rounded-md border border-primary/20 space-y-3 overflow-hidden animate-in fade-in slide-in-from-top-1`}
    >
      {/* Header & Mode Toggles */}
        ...
        <div className="space-y-4 animate-in fade-in duration-300">
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between group/cmd bg-black/20 p-1.5 rounded border border-white/5">
                <code className="text-muted-foreground truncate flex-1 mr-2">
                  {heuristics.av1.ffmpeg}
                </code>
                <button
                  onClick={() => copyToClipboard(heuristics.av1.ffmpeg, 'h-av1-f')}
              ...
              <div className="text-muted-foreground italic truncate px-1 opacity-70">
                {heuristics.av1.handbrake}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between group/cmd bg-black/20 p-1.5 rounded border border-white/5">
                <code className="text-muted-foreground truncate flex-1 mr-2">
                  {heuristics.hevc.ffmpeg}
                </code>
                <button
                  onClick={() => copyToClipboard(heuristics.hevc.ffmpeg, 'h-hevc-f')}
                  className="p-1 hover:text-primary transition-colors"
                >
                  {copied === 'h-hevc-f' ? (
                    <Check className="w-3.5 h-3.5" />
        ...
        <div className="py-8 flex flex-col items-center justify-center gap-3 text-muted-foreground">
          ...
          <div className="text-center">
            <p className="font-medium text-foreground">
              Gemini is performing bit-depth and codec audit...
            </p>
            <p className="text-[10px] opacity-70">
              Calculating optimal CRF for {item.resolution} source
            </p>
          </div>
        </div>
      ) : aiAdvice ? (
